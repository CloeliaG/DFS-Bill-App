...
const useBillStore = defineStore('bill', {
  ...
  actions: {
    ...
    // récupère les n dernières factures
    async getLastNBills(num) {
      try {
        // requête à l'API GET /bills
        const response = await axios.get(`/bills/last/${num}`)
    ...
    createBill() {
      ...
      // j'ai dû commenté le delete sinon il ne m'affichait pas le formulaire
