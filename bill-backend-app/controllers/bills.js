...
// récupère les n derniers bills
const getLastNBills = (req, res)=> {
  const nLast = req.params.num
  Bill.find().sort({ _id: -1 }).limit(nLast)
  .then((bills)=>{
      res.status(200).json(bills)
  }).catch(error => res.status(400).json({ error }))
}
...
module.exports = { getItems, getLastNBills, getItem, patchItem, postItem, deleteItem }